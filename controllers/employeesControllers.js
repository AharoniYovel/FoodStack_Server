...
exports.employeeInfo = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(500).json({
      err_msg: "There is problem in the server, try again later1",
      err,
    });
  }
};
...
exports.addAdmin = async (req, res) => {
  ...
  try {
    ...
    if (err.code)
      res
        .status(500)
        .json({ err_msg: "There is problem in the server, try again later12" });
  }
};
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    res.json({
      token,
      employee: { nickName: employee.nickName, role: employee.role },
    });
  } catch (err) {
    res.status(500).json({
      err_msg: "There is problem in the server, try again later1 and db",
    });
  }
};
...
exports.editEmployee = async (req, res) => {
  ...
  if (
    req.params.idEdit == infConect.superAdminId ||
    req.params.idEdit == req.tokenData._id
  ) {
    return res
      .status(401)
      .json({ msg_err: "You cant change yourself and the super Admin!!!" });
  }
  ...
  try {
    ...
    if ((err.code = 11000)) {
      return res
        .status(500)
        .json({ err_msg: "Email is already in system", code: 11000 });
    }
...
exports.deleteEmployee = async (req, res) => {
  try {
    ...
    if (idDel == infConect.superAdminId || idDel == req.tokenData._id) {
      return res
        .status(401)
        .json({ msg_err: "You cant delete yourself and the super Admin!!!" });
    }
